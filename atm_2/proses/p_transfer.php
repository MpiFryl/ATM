...
$sql = "SELECT saldo, pin FROM users WHERE username='$username_pengirim'";
...
if ($result->num_rows > 0) {
    ...
    $query1 = "UPDATE users SET saldo=$saldo_pengirim_baru WHERE username='$username_pengirim'";

    // Tambahkan saldo ke penerima
    $saldo_penerima_baru = $saldo_penerima + $jumlah;
    $query2 = "UPDATE users SET saldo=$saldo_penerima_baru WHERE username='$tujuan'";

    if ($conn->query($query1) && $conn->query($query2)) {
        echo "<script>alert('Transfer berhasil!'); window.location.href='../menu.php';</script>";
    } else {
